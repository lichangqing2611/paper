...
我们的代码示例使用einsum表示法（如在TensorFlow和numpy中定义的），用于任意维度张量之间的广义收缩（generalized contractions）。在此表示法中，一个方程命名输入和输出张量的维度。该计算在数值上等同于将每个输入广播（Broadcasting）到具有所有维度的并集，逐分量相乘（Multiplying component-wise），并对所有不在期望输出形状中的维度求和（Summing across）。例如：

```python
k,mk    -B-> mk,mk       -M-> mk    -S-> m
d,hdk   -B-> hdk,hdk     -M-> hdk   -S-> hk
md,hdk  -B-> mhdk,mhdk   -M-> mhdk  -S-> mhk  -T-> hmk
bnd,hdk -B-> bnhdk,bnhdk -M-> bnhdk -S-> bnhk -T-> bhnk
```
